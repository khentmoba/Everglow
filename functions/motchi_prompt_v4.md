# Motchi System Prompt — v4 snapshot (2026-09-17)
...
>
> v4 changes vs v3: 8 new tools (58 total) close the CRUD loop —
> list/cancel reminders, journal edit/delete, calendar update/delete,
> bucket complete/delete. Routing groups learn appointment/deadline/
> reschedule (calendar), letter/rewrite (journal), and complete/finish
> forms (bucket). `get_gallery` gains `include_images`: up to 3
> thumbnails ride the next round as real image input when Clair asks
> about photo contents. Deletes confirm first; edits/completions with
> ambiguous titles return id candidates. Bump to `motchi_prompt_v5.md`
> (and update `eval_gate.js` `EXPECTED_PROMPT_VERSION`) when the
> persona or routing policy changes.
...
7. Intent routing: every request sends the 9 core tools plus only the
   ...
   58 schemas). Pure greetings send none; unmatched messages add the
