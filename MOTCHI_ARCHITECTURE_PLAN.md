...
**Goal:** Motchi can *do things* — add movies, save notes, set moods, search TMDB, create reminders — via structured tool calls, not just talk.

### 1A. Tool Schemas (`functions/index.js`)

Define `MOTCHI_TOOLS` array with OpenAI-compatible function schemas:
...
The proxy absorbs all tool complexity — client still only sees `reasoning` and `content` deltas. Optional: add `tool_status` events for UX ("Motchi is searching...").

### 1E. System Prompt Update

Add tool awareness to Motchi's persona:
...
| File | Change |
|------|--------|
| `functions/index.js` | Add `MOTCHI_TOOLS`, `executeTool()`, rewrite streaming as agent loop, update persona |
...
**Goal:** Motchi initiates conversations — daily digest, mood check-ins, special day nudges.

### 3A. Daily Digest (8:00 AM PHT)

`motchiDailyDigest` — Cloud Scheduler → gather context → call Agnes for a short digest → FCM push to both users.

### 3B. Mood Check-In (8:00 PM PHT)

`motchiMoodCheckIn` — check who hasn't logged mood today → FCM nudge.

### 3C. Special Day Nudges (9:00 AM PHT)

`motchiSpecialDayNudge` — anniversary (Feb 14), birthdays (Oct 26, Feb 21) → FCM nudge within 7 days.
...
- [ ] Ask Motchi "add Interstellar to our watchlist" → verify Firestore write
- [ ] Ask Motchi "save a note saying I love you" → verify Starlight Jar entry
- [ ] Ask Motchi "I'm feeling happy today" → verify mood logged
- [ ] Ask Motchi "what's the weather?" → verify weather response
...
### Phase 4
- [ ] Ask Motchi for movie recommendations → verify real titles only
