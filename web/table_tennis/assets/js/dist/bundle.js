...
function updateScore(_player) {
    ...
    if(_player == "user") {
        ...
        mj_score++;
        mj_score_row++;
        if (mj_score_row>mj_best_score_row) mj_best_score_row=mj_score_row;
        //console.log('score '+mj_score);
        //console.log('score row '+mj_score_row);
        //console.log('mj_kk___'+oGameData.gameId);
...
function initGameComplete() {
    ...
    if (oGameData.userScore > oGameData.enemyScore) {
        ...
        mj_win++;
        mj_win_row++;
        //console.log('win '+mj_win);
        lechuck.stat.put(function(response){}, 'win_current',mj_win);
        lechuck.stat.put(function(response){}, 'win_total',1);
        //console.log('win row '+mj_win_row);
        lechuck.stat.put(function(response){}, 'win_row',mj_win_row);
        //console.log('score '+mj_score);
        //lechuck.stat.put(function(response){}, 'score_current',mj_score);
        //lechuck.stat.put(function(response){}, 'score_total',mj_score);
        //console.log('best score row '+mj_best_score_row);
        lechuck.stat.put(function(response){}, 'score_row',mj_best_score_row);
       
        won_matches_in_series_opponent = 0;
        won_matches_in_series_player++;
        if(won_matches_in_series_player > 1) {
            //localStorage.setItem('won_matches_in_series_player',won_matches_in_series_player);
        ...
        if(oGameData.enemyScore == 0) {
            ...
            //localStorage.setItem('flawless_victory',1);
        }

        if((oGameData.gameId+1) >= 6) {
            window.famobi_analytics.trackStats("cup_won");
            //localStorage.setItem('cup',1);
            mj_cup++;
            //console.log('cup '+mj_cup);
            lechuck.stat.put(function(response){}, 'cup_current',mj_cup);
            lechuck.stat.put(function(response){}, 'cup_total',1);
           // console.log('mj_kk___'+oGameData.gameId);
           // localStorage.setItem('kk',oGameData.gameId);
        }

    } else {
        window.famobi_analytics.trackStats("match_lost");
       // console.log('mj_lost');
       lechuck.stat.put(function(response){}, 'lose',1);
        //console.log('score '+mj_score);
        //console.log('score row '+mj_score_row);
        mj_score_row=0;
       // console.log('best score row '+mj_best_score_row);
       lechuck.stat.put(function(response){}, 'score_row',mj_best_score_row);
        ...
        if(won_matches_in_series_opponent > 1) {
           // localStorage.setItem('won_matches_in_series_opponent',won_matches_in_series_opponent);
            window.famobi_analytics.trackStats("won_matches_in_series_opponent", won_matches_in_series_opponent);
        }

        if(oGameData.userScore == 0) {
           // localStorage.setItem('flawless_defeat',1);
...
// --- Everglow placeholder patch (from assets/index.html:43) ---
(function(){
  function makePlaceholder(){ var c=document.createElement('canvas'); c.width=64; c.height=64; return c; }
  if(typeof Utils!=='undefined'&&Utils.AssetLoader){
    var _orig=Utils.AssetLoader.prototype.loadImage;
    Utils.AssetLoader.prototype.loadImage=function(_oData){ var _this=this; var img=new Image(); img.onload=function(){ _this.oAssetData[_oData.id]=_this.oAssetData[_oData.id]||{}; _this.oAssetData[_oData.id].img=img; _this.oAssetData[_oData.id].loaded=true; _this.oAssetData[_oData.id].oData=_this.oAssetData[_oData.id].oData||{}; _this.oAssetData[_oData.id].oData.oAtlasData=_oData.oAtlasData||{}; ++_this.assetsLoaded; _this.checkLoadComplete(); }; img.onerror=function(){ var ph=makePlaceholder(); _this.oAssetData[_oData.id]=_this.oAssetData[_oData.id]||{}; _this.oAssetData[_oData.id].img=ph; _this.oAssetData[_oData.id].loaded=true; _this.oAssetData[_oData.id].oData=_this.oAssetData[_oData.id].oData||{}; _this.oAssetData[_oData.id].oData.oAtlasData=_oData.oAtlasData||{}; ++_this.assetsLoaded; _this.checkLoadComplete(); }; img.src=_oData.file; };
    Utils.AssetLoader.prototype.loadJSON=function(_oData){ var _this=this; var xobj=new XMLHttpRequest(); xobj.open('GET',_oData.file,true); xobj.onreadystatechange=function(){ if(xobj.readyState==4){ if(xobj.status==200){ try{ _this.textData[_oData.id]=JSON.parse(xobj.responseText);}catch(e){} } ++_this.assetsLoaded; _this.checkLoadComplete(); } }; xobj.onerror=function(){ ++_this.assetsLoaded; _this.checkLoadComplete(); }; xobj.send(null); };
    var _origAH=Utils.UserInput.prototype.addHitArea; Utils.UserInput.prototype.addHitArea=function(_id,_func,_oD,_sT,_oO,_bF){ if(_oO&&_oO.oImgData&&_oO.oImgData.oData&&_oO.oImgData.oData.oAtlasData){ var atlas=_oO.oImgData.oData.oAtlasData; var fid=_oO.id; if(fid!=null&&!atlas[fid]) atlas[fid]={x:0,y:0,width:64,height:64}; } return _origAH.call(this,_id,_func,_oD,_sT,_oO,_bF); };
  }
})();
(function () {
