import { USER_COUNTRIES, ENEMY_CUPS, SPARE_ENEMY, INITIAL_GAME_DATA, SAVE_KEY } from '../utils/constants.js';
...
export class Game {
    constructor(canvasId = 'canvas') {
        ...
        this.oGameData = { ...INITIAL_GAME_DATA };
        this.oImageIds = {};
        this.firstRun = true;
        this.controlState = 0;
        this.flagPage = 0;
        this.justWonCup = false;
        this.rallyHits = 0;
        this.isMobile = false;
        this.isBugBrowser = false;
        this.muted = false;
        this.hasFocus = true;
        this.audioType = 0;
        this.swipeState = 0;
        this.startTouchY = 0;
        this.saveDataHandler = new SaveDataHandler(SAVE_KEY);
        this.countryFlags = new CountryFlags(USER_COUNTRIES, false);
        ...
        window['ctx'] = this.ctx;
        window['delta'] = this.delta;
        window['oGameData'] = this.oGameData;
        window['oImageIds'] = this.oImageIds;
        window['gameState'] = this.gameState;
    }
    // -------------------------------------------------------------------------
    // Boot
    // -------------------------------------------------------------------------
    extGameLoad() { this.loadPreAssets(); }
    loadPreAssets() {
        ...
        window['preAssetLib'] = this.preAssetLib;
    }
    loadAssets() {
        let mg = 'images/BrandingPlaceholderButton.png';
        try {
            mg = getFamobi().getMoreGamesButtonImage();
        }
        catch { }
        const specs = [
        ...
        this.updateLoaderEvent();
    }
    syncGlobals() {
        const w = window;
        w['gameState'] = this.gameState;
        w['oGameData'] = this.oGameData;
        w['oImageIds'] = this.oImageIds;
        w['delta'] = this.delta;
        w['canvasScale'] = this.canvasScale;
        w['rallyHits'] = this.rallyHits;
        w['firstRun'] = this.firstRun;
        w['isMobile'] = this.isMobile;
        w['assetLib'] = this.assetLib;
        w['tableTop'] = this.tableTop;
        w['userBat'] = this.userBat;
        w['enemyBat'] = this.enemyBat;
        w['ball'] = this.ball;
    }
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    initSplash() {
        const famobi = getFamobi();
        const analytics = ensureAnalytics();
        window.famobi_onPauseRequested = () => { try {
            Howler.mute(true);
            this.music?.pause();
        }
        catch { } };
        window.famobi_onResumeRequested = () => {
            if (!this.muted && this.gameState !== 'pause') {
                try {
                    Howler.mute(false);
                    this.music?.play();
                }
        ...
        if (uid === 1234) {
            ...
            this.firstRun = false;
            this.oGameData.userId = uid;
        }
        this.controlState = this.saveDataHandler.getControlState();
        try {
            famobi.onRequest('pauseGameplay', () => { if (this.gameState === 'game')
                this.butEventHandler('pause'); });
            famobi.onRequest('resumeGameplay', () => { if (this.gameState === 'pause')
                this.butEventHandler('playFromPause'); });
        }
        catch { }
        this.syncGlobals();
    ...
    initGameIntro() {
        if (this.oGameData.cupId === 10 && this.oGameData.gameId === 6) {
            this.oGameData.cupId = 9;
            this.oGameData.gameId = 5;
        }
        ...
        if (this.oGameData.enemyId === this.oGameData.userId)
            this.oGameData.enemyId = this.countryFlags.getIdFromISO(SPARE_ENEMY);
        this.gameState = 'gameIntro';
        this.syncGlobals();
        this.previousTime = Date.now();
        this.updateGameIntroScreenEvent();
    }
    /** Starts a match — replaces _initGame game.js:3473 */
    initGame(_restart = false) {
    ...
    _initGame() {
        this.gameState = 'game';
        this.syncGlobals();
        this.oGameData.userScore = 0;
        this.oGameData.enemyScore = 0;
        this.justWonCup = false;
        this.background = new Background(() => this.oGameData.cupId);
        ...
        this.ball = new Ball(() => this.canvas, () => this.ctx, () => this.tableTop, () => this.userBat, () => this.enemyBat, (who) => this.updateScore(who));
        window['ball'] = this.ball;
        window['tableTop'] = this.tableTop;
        window['userBat'] = this.userBat;
        window['enemyBat'] = this.enemyBat;
        this.syncGlobals();
        this.previousTime = Date.now();
    ...
    getDelta() {
        const now = Date.now();
        let d = (now - this.previousTime) / 1000;
    ...
    updateGameEvent() {
        ...
        if ((this.ball.offTable || this.ball.offSide) || (this.ball.height < 0 && this.ball.tablePosY < 0.5 && (this.ball.tablePosX < -1 || this.ball.tablePosX > 1))) {
            ...
            this.tableTop.render(this.canvas, this.ctx);
            this.enemyBat.render(this.ctx);
            this.tableTop.renderNet(this.canvas, this.ctx);
        }
        else if (this.ball.tablePosY > 0.5) {
            this.tableTop.render(this.canvas, this.ctx);
            this.enemyBat.render(this.ctx);
            this.tableTop.renderNet(this.canvas, this.ctx);
            this.ball.render(this.ctx);
        ...
        this.userBat.update(this.delta);
        this.userBat.render(this.ctx, this.canvas);
    ...
    initGameComplete(_won) {
        this.gameState = 'gameComplete';
        this.syncGlobals();
        // promotion mirrors game.js:4275
        if (_won) {
            this.oGameData.gameId++;
            if (this.oGameData.gameId >= 6) {
                this.oGameData.cupId++;
                this.justWonCup = true;
                if (this.oGameData.cupId > 9) {
                    this.oGameData.cupId = 10;
                    this.oGameData.gameId = 6;
                }
                else
                    this.oGameData.gameId = 0;
            }
            this.saveDataHandler.setGameData(this.oGameData);
            this.saveDataHandler.saveData();
        }
    }
    // -------------------------------------------------------------------------
    // Input dispatch (subset of butEventHandler game.js:3722)
    // -------------------------------------------------------------------------
    butEventHandler(id, _data) {
        switch (id) {
            case 'pause':
            ...
            case 'playFromPause':
                ...
                break;
            case 'playFromStart':
                this.firstRun ? this.initChooseCountry() : this.initGameIntro();
                break;
            case 'playFromGameIntro':
                this.initGame();
                break;
            case 'changeCountryFromStart':
                this.initChooseCountry();
                break;
            case 'cupsFromStart':
                this.initMapScreen();
                break;
    ...
    initChooseCountry() { this.gameState = 'chooseCountry'; this.syncGlobals(); }
    initMapScreen() { this.gameState = 'map'; this.syncGlobals(); }
    resizeCanvas() {
        ...
        if (w > h) {
            if (h < 500) {
                ...
                this.canvas.width = 500 * (w / h);
                this.canvasScale = 500 / h;
            }
            else if (h > 700) {
                this.canvas.height = 700;
                this.canvas.width = 700 * (w / h);
                this.canvasScale = 700 / h;
            }
            else
                this.canvasScale = 1;
        }
            ...
            if (w < 500) {
                ...
                this.canvas.height = 500 * (h / w);
                this.canvasScale = 500 / w;
            }
            else if (w > 700) {
                this.canvas.width = 700;
                this.canvas.height = 700 * (h / w);
                this.canvasScale = 700 / w;
            }
            else
                this.canvasScale = 1;
        }
        window['canvasScale'] = this.canvasScale;
        window.scrollTo(0, 0);
    }
}
