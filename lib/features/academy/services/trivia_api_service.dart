...
class TriviaApiService {
  ...
  Future<void> initSession() async {
    ...
    if (_sessionToken == null || timestamp == null || 
        DateTime.now().millisecondsSinceEpoch - timestamp > 6 * 60 * 60 * 1000) {
  ...
  Future<List<AcademyQuestion>> fetchQuestions({
    ...
    final url = '$_baseUrl?amount=$amount&category=$categoryId&type=multiple&token=$_sessionToken';
    ...
    if (response.statusCode == 200) {
      ...
      switch (responseCode) {
        case 0: // Success
          ...
          return results.map((json) => _mapJsonToQuestion(json, categoryId)).toList();
  ...
  AcademyQuestion _mapJsonToQuestion(Map<String, dynamic> json, int categoryId) {
