  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        // The table lists the songs we can see; the headline numbers are the
        // exact all-time counts. When plays exist outside this list they are
        // shown as one row so the column still adds up to the total.
        if (showdown.hasOtherPlays && (expanded || tracks.length <= _collapsedCount))
          _OtherSongsRow(showdown: showdown),
      ],
    );
  }
}

/// Aggregate row for plays the song table cannot name: songs outside the
/// fetched all-time top tracks. Keeps the column honest against the exact
/// total from Last.fm instead of silently under-reporting.
class _OtherSongsRow extends StatelessWidget {
  final ArtistShowdownProvider showdown;
  const _OtherSongsRow({required this.showdown});

  @override
  Widget build(BuildContext context) {
    final khent = showdown.khentOtherPlays;
    final clair = showdown.clairOtherPlays;
    final leader = khent > clair
        ? 'khent'
        : clair > khent
        ? 'clair'
        : null;
    final leaderColor = switch (leader) {
