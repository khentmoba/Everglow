...
class JournalService {
  ...
  Stream<List<JournalEntry>> watchAll() {
    return withFirestoreTimeout(
      _db.collection(_collection).orderBy('createdAt', descending: true).limit(100).snapshots().map(
            (snap) => snap.docs.map((d) => JournalEntry.fromFirestore(d)).toList(),
          ),
      label: 'journal-all',
    );
  }
  ...
  Stream<List<JournalEntry>> watchPinned() {
    return withFirestoreTimeout(
      _db.collection(_collection).where('isPinned', isEqualTo: true).orderBy('createdAt', descending: true).limit(20).snapshots().map(
  ...
  Stream<List<JournalEntry>> watchByCategory(JournalCategory cat) {
    return withFirestoreTimeout(
      _db.collection(_collection).where('category', isEqualTo: cat.name).orderBy('createdAt', descending: true).limit(50).snapshots().map(
            (s) => s.docs.map((d) => JournalEntry.fromFirestore(d)).toList(),
          ),
  ...
  Stream<List<JournalEntry>> watchByAuthor(String author) {
    return withFirestoreTimeout(
      _db.collection(_collection).where('author', isEqualTo: author.toLowerCase()).orderBy('createdAt', descending: true).limit(50).snapshots().map(
            (s) => s.docs.map((d) => JournalEntry.fromFirestore(d)).toList(),
          ),
  ...
  Stream<List<JournalEntry>> search(String query) {
    ...
    return _db.collection(_collection).orderBy('createdAt', descending: true).limit(80).snapshots().map(
          (snap) => snap.docs
  ...
  Future<List<JournalEntry>> getOnThisDay() async {
    ...
    try {
      final snap = await _db.collection(_collection).where('monthDay', isEqualTo: md).limit(50).get();
      final entries = snap.docs.map((d) => JournalEntry.fromFirestore(d)).where((e) => e.createdAt.year != now.year).toList();
      if (entries.isNotEmpty) return entries;
      // Fallback: client filter
      final fallback = await _db.collection(_collection).orderBy('createdAt', descending: true).limit(200).get();
      return fallback.docs.map((d) => JournalEntry.fromFirestore(d)).where((e) => e.createdAt.month == now.month && e.createdAt.day == now.day && e.createdAt.year != now.year).toList();
    } catch (e) {
  ...
  Future<void> togglePin(String id, bool pinned) async {
    try {
      await _db.collection(_collection).doc(id).update({'isPinned': pinned, 'updatedAt': Timestamp.now()});
    } catch (e) {
  ...
  Future<void> toggleLock(String id, bool locked) async {
    try {
      await _db.collection(_collection).doc(id).update({'isLocked': locked, 'updatedAt': Timestamp.now()});
    } catch (e) {
  ...
  Stream<Map<String, int>> watchHeatmap({int days = 90}) {
    ...
    return _db.collection(_collection).where('createdAt', isGreaterThanOrEqualTo: Timestamp.fromDate(start)).snapshots().map((snap) {
